...
export default function DocentesPage() {
  ...
  const [selectedDetail, setSelectedDetail] = useState<DocenteDetail | null>(null);
  ...
  const fetchDocentes = async (query?: { nombre?: string; nomina?: string | number }) => {
    setLoading(true);
    setError("");
    try {
  ...
  const handleViewDetail = async (nomina: number) => {
    ...
    try {
      ...
      const docenteData =
        docenteRes.status === "fulfilled" ? docenteRes.value?.data?.docente : null;
      const horario: HorarioEntry[] =
        horarioRes.status === "fulfilled" ? horarioRes.value?.data?.horario ?? [] : [];
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false);
      setFormData({ nomina: 0, nombres: "", apellidos: "", correo: "", cargo: "PTC" });
      await fetchDocentes();
    } catch (err) {
      if (axios.isAxiosError(err)) {
    ...
    <>
      <div className="topbar">
        <div>
          ...
          <p className="topbar-sub">
            {loading ? "Cargando..." : `${docentes.length} docentes registrados`}
          </p>
        </div>
      ...
      {showForm && (
        ...
        >
          ...
          {formError && (
            <p style={{ color: "red", fontSize: 13, marginBottom: 8 }}>{formError}</p>
          )}
          <form
          ...
          >
            <div>
              <label style={{ fontSize: 12, color: "var(--gray-600)" }}>Nómina</label>
              <input
                type="number"
                required
                ...
                onChange={(e) => setFormData({ ...formData, nomina: Number(e.target.value) })}
              />
            </div>
            <div>
              <label style={{ fontSize: 12, color: "var(--gray-600)" }}>Nombres</label>
              <input
                type="text"
                required
                className="search-input"
            ...
            <div>
              <label style={{ fontSize: 12, color: "var(--gray-600)" }}>Apellidos</label>
              <input
                type="text"
                required
                className="search-input"
            ...
            <div>
              <label style={{ fontSize: 12, color: "var(--gray-600)" }}>Correo</label>
              <input
                type="email"
                required
                className="search-input"
            ...
            <div>
              <label style={{ fontSize: 12, color: "var(--gray-600)" }}>Cargo</label>
              <select
                required
                className="search-input"
                style={{ width: "100%", marginTop: 4 }}
                ...
                onChange={(e) =>
                  setFormData({ ...formData, cargo: e.target.value as DocenteCreateBody["cargo"] })
                }
              >
                {CARGO_OPTIONS.map((c) => (
                          ...
                          >
                            {detailLoading ? (
                              ...
                              >
                                    ...
                                    <ul style={{ marginTop: 8, paddingLeft: 0, listStyle: "none" }}>
                                      {selectedDetail.horario.map((h, i) => (
                                        <li key={i} style={{ marginBottom: 4 }}>
                                          ...
                                          {" — "}
                                          {new Date(h.inicio).toLocaleDateString("es-MX", {
                                            weekday: "long",
                                          })}
                                          {" "}
                                          {new Date(h.inicio).toLocaleTimeString("es-MX", {
                                            hour: "2-digit",
                                            minute: "2-digit",
                                          })}
                                          {" – "}
                                          {new Date(h.fin).toLocaleTimeString("es-MX", {
                                            hour: "2-digit",
                                            minute: "2-digit",
                                          })}
                                        </li>
                                ...
                                <div>
                                  <strong>Estadísticas de calificaciones</strong>
                                  {!selectedDetail.stats ? (
                                    <p style={{ color: "var(--gray-500)" }}>Sin estadísticas</p>
                                  ) : (
                                    <div style={{ marginTop: 8 }}>
                                      <p>Promedio: <strong>{selectedDetail.stats.promedio?.toFixed(1)}</strong></p>
                                      <p>Acreditados: <strong>{selectedDetail.stats.acreditados}</strong></p>
                                      <p>Reprobados: <strong>{selectedDetail.stats.reprobados}</strong></p>
                                      <p>Total calificaciones: <strong>{selectedDetail.stats.total}</strong></p>
                                    </div>
                                  )}
                                </div>
                              </div>
